            ...
            if not override:
                if self.mode == 1 or z_wait:  # override CMDS
                    ...
                    if self.control_mode == SURFACE_CTRL:
                        ...
                        # POSHOLD MODE
                        if not override_z and not z_wait:
                            vz = PH_HOVER
...
def set_gps(board,
    ...
    if mslAltitude > 2147483647:
        logging.warning(f'GPS altitude invalid')
        mslAltitude = 0
...
class CogniflyController:
    ...
    def _controller(self, screen, profiling_duration=-1):
        ...
        try:
            ...
            print("DEBUG trying to get MSPy object")
            with MSPy(device=self.device_str, loglevel='WARNING', baudrate=115200, timeout=0, min_time_between_writes=0.02) as board:
