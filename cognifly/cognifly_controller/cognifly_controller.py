...
class CogniflyController:
    ...
    def _update_pose(self, board, screen, retrieve_all):
        ...
        elif self.no_barometer:
            set_barometer_from_altitude(board=board, altitude=0.05, t_start=self._t_start)  # Fake constant barometer
        if write_rangefinder:
            set_rangefinder_from_altitude(board=board, altitude=pos_z_wf)
        if write_compass:
            sent_yaw = 0.0 if self.no_yaw else yaw
            set_compass_from_yaw(board=board, yaw=sent_yaw, t_start=self._t_start, yaw_offset=self.compass_offset)
        if write_gps:
    ...
    def _flight(self, screen):
        ...
        if self.trace_logs:
            ...
            self.x_tracer.write_line(f"{now};{pos_x_wf};{vel_x_wf};{vel_x_df};{self.pid_vel_x.setpoint};{int(self.pid_vel_x._auto_mode)};{x_target}")
            self.y_tracer.write_line(f"{now};{pos_y_wf};{vel_y_wf};{vel_y_df};{self.pid_vel_y.setpoint};{int(self.pid_vel_y._auto_mode)};{y_target}")
            self.z_tracer.write_line(f"{now};{pos_z_wf};{vel_z_wf};{vel_z_df};{self.pid_vel_z.setpoint};{int(self.pid_vel_z._auto_mode)};{z_target}")
            self.w_tracer.write_line(f"{now};{yaw};{yaw_rate};{yaw_rate};{self.pid_w_z.setpoint};{int(self.pid_w_z._auto_mode)};{w_target}")
